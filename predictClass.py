...
class PredictProcessor():
    ...
    def _create_joint_tensor(self, tensor, name = 'joint_tensor',debug = False):
        ...
        with tf.name_scope(name):
            ...
            if debug:
                print(shape)
            if len(shape) == 3:
                resh = tf.reshape(tensor[:,:,0], [-1])  # Flattens to 1D
            elif len(shape) == 4:
                resh = tf.reshape(tensor[-1,:,:,0], [-1])  # Flattens to 1D
            if debug:
                print(resh)
            arg = tf.arg_max(resh,0)  # Find the position of the highest element in the first channel
            if debug:
                print(arg, arg.get_shape(), arg.get_shape().as_list())
            joints = tf.expand_dims(tf.stack([arg // tf.to_int64(shape[1]), arg % tf.to_int64(shape[1])], axis = -1), axis = 0)  # Get row/column of the first joint
    ...
    def predict_coarse(self, img, debug = False, sess = None):
        ...
        if img.shape == (256,256,3):
            if sess is None:
                out = self.HG.Session.run(self.HG.output, feed_dict={self.HG.img_source : np.expand_dims(img, axis = 0)})
            else:
                out = sess.run(self.HG.output, feed_dict={self.HG.img_source : np.expand_dims(img, axis = 0)})
        else:
    ...
    def pred(self, img, debug = False, sess = None):
        ...
        if img.shape == (256,256,3):
            if sess is None:
                out = self.HG.Session.run(self.HG.pred_sigmoid, feed_dict={self.HG.img_source : np.expand_dims(img, axis = 0)})
            else:
                out = sess.run(self.HG.pred_sigmoid, feed_dict={self.HG.img_source : np.expand_dims(img, axis = 0)})
        else:
            print('Image Size does not match placeholder shape')
            raise Exception
        if debug:
    ...
    def joints_pred(self, img, coord = 'hm', debug = False, sess = None):
        ...
        if debug:
            t = time()
            if sess is None:
                j1 = self.HG.Session.run(self.HG.joint_tensor, feed_dict = {self.HG.img_source: img})
            else:
                j1 = sess.run(self.HG.joint_tensor, feed_dict = {self.HG.img_source: img})
            print('JT:', time() - t)
            t = time()
            if sess is None:
                j2 = self.HG.Session.run(self.HG.joint_tensor_final, feed_dict = {self.HG.img_source: img})
            else:
                j2 = sess.run(self.HG.joint_tensor_final, feed_dict = {self.HG.img_source: img})
            print('JTF:', time() - t)
            if coord == 'hm':
                return j1, j2
            elif coord == 'img':
            ...
            if sess is None:
                j = self.HG.Session.run(self.HG.joint_tensor_final, feed_dict = {self.HG.img_source: img})
            else:
                j = sess.run(self.HG.joint_tensor_final, feed_dict = {self.HG.img_source: img})
            if coord == 'hm':
                return j
            elif coord == 'img':
    ...
    def joints_pred_numpy(self, img, coord = 'hm', thresh = 0.2, sess = None):
        ...
        """
        if sess is None:
            hm = self.HG.Session.run(self.HG.pred_sigmoid, feed_dict = {self.HG.img_source: img})
        else:
            hm = sess.run(self.HG.pred_sigmoid, feed_dict = {self.HG.img_source: img})
    ...
    def batch_pred(self, batch, debug = False):
        ...
        if batch[0].shape == (256,256,3):
            out = self.HG.Session.run(self.HG.pred_sigmoid, feed_dict={self.HG.img_source : batch})
        else:
            print('Image Size does not match placeholder shape')
            raise Exception
        if debug:
        ...
        for i in range(len(self.links)):
            ...
            for p in l:
                if np.array_equal(joints[p], [-1,-1]):
                    good_link = False
            if good_link:
                position = PredictProcessor.givePixel(self.links[i]['link'],joints)
                cv2.line(img, tuple(position[0])[::-1], tuple(position[1])[::-1], self.links[i]['color'][::-1], thickness = 2)
        if joint_plt:
    ...
    def pltSkeleton(self, img, thresh = 0.2, pltJ = True, pltL = True, tocopy = True, norm = True):
        ...
        if norm:
            img_hg = img / 255
        hg = self.HG.Session.run(self.HG.pred_sigmoid, feed_dict = {self.HG.img_source: np.expand_dims(img_hg, axis = 0)})
    ...
    def reconstructACPVideo(self, load = 'F:/Cours/DHPE/DHPE/hourglass_tiny/withyolo/p4frames.mat',n = 5):
        ...
        while True:
            ...
            j = self.HG.Session.run(self.HG.joint_tensor_final, feed_dict = {self.HG.img_source: np.expand_dims(img_hg / 255, axis = 0)})
    ...
    def hpeWebcam(self, thresh = 0.6, plt_j = True, plt_l = True, plt_hm = False, debug = True):
        ...
        while True:
            ...
            img_hg = cv2.cvtColor(img_hg, cv2.COLOR_BGR2RGB)
            hg = self.HG.Session.run(self.HG.pred_sigmoid, feed_dict = {self.HG.img_source: np.expand_dims(img_hg / 255, axis = 0)})
            j = np.ones(shape = (self.params['num_joints'],2)) * -1
            if plt_hm:
    ...
    def mpe(self, j_thresh = 0.5, nms_thresh = 0.5, plt_l = True, plt_j = True, plt_b = True, img_size = 800,  skeleton = False):
        ...
        while True:
            ...
            for box in results_person:
                ...
                hm = self.HG.Session.run(self.HG.pred_sigmoid, feed_dict={self.HG.img_source: np.expand_dims(img_person / 255, axis= 0)})
